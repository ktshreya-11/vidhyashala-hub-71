...
export const Route = createFileRoute("/login")({
  head: () => ({
    meta: [
      { title: "Login — Vidyashala Hub" },
      { name: "description", content: "Sign in to Vidyashala Hub as a Student or Professional." },
    ],
  }),
  component: LoginPage,
});
...
function LoginPage() {
  ...
  return (
    <div className="min-h-screen bg-gradient-hero">
      <div className="mx-auto grid max-w-6xl gap-10 px-6 py-10 lg:grid-cols-[1fr_minmax(380px,420px)] lg:py-16">
        <div className="hidden flex-col justify-between rounded-3xl border border-white/10 bg-black/30 p-10 text-white lg:flex">
          <div>
            <BackButton variant="dark" fallback="/" />
            <h1 className="mt-8 font-display text-5xl font-bold leading-tight">
              Welcome back to <br /><span className="bg-gradient-to-r from-primary-glow to-white bg-clip-text text-transparent">Vidyashala</span>
            </h1>
            <p className="mt-4 max-w-md text-white/70">Pick up where you left off — your hubs, badges and projects are waiting.</p>
          </div>
          <div className="flex items-end justify-between">
            <Mascot variant="owl" size={180} />
            <Mascot variant="robot" size={140} />
          </div>
        </div>
        ...
        <div className="rounded-3xl border border-border bg-card p-8 shadow-elegant">
          ...
          <div className="mb-6 flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-xs text-muted-foreground">Use Google, email, or phone</div>
            </div>
          </div>
          ...
          <div className="mb-3 flex rounded-lg border border-border p-1 text-xs">
            {(["email", "phone"] as const).map((t) => (
              <button
                ...
                onClick={() => setTab(t)}
                type="button"
                className={"flex-1 rounded-md py-1.5 font-semibold capitalize transition-colors " + (tab === t ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:text-foreground")}
              >
                {t}
              </button>
            ))}
          ...
          <form onSubmit={submit} className="space-y-3">
            ...
            <div>
              <Label className="mb-1 block text-xs">Password {tab === "phone" && "/ OTP"}</Label>
              <div className="relative">
